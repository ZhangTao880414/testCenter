...
class NodeInfoAdmin(admin.ModelAdmin):
    ...
    def gen_node_cfg_module(self, request, queryset):
        ...
        self.message_user(request, '生产模板中，请稍后查询')
...
class ToraMqAdmin(admin.ModelAdmin):
    list_display = ('id', 'mq_group', 'business', 'mq_type', 'ip_addr', 'port')
...
class NodeComponentInfoAdmin(admin.ModelAdmin):
    list_display = ('id', 'node', 'business', 'ip_addr', 'component', 'port', 'current_version', 'upgrade_time', 'sys_status', 'monitor_time')
admin.site.register(models.NodeComponentInfo, NodeComponentInfoAdmin)
