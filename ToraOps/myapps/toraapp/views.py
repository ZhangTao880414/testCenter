...
class ShelfMachineList(generics.ListCreateAPIView):
    ...
    permission_classes = (IsAuthenticated,)
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ('engine_room', 'cabinet', 'purpose', 'assert_type', 'serial_number', 
...
class NodeComponentInfoViewSet(viewsets.ModelViewSet):
    queryset = models.NodeComponentInfo.objects.all()
    serializer_class = tora_ser.NodeComponentInfo_Serializers
    permission_classes = (IsAdminUser,)
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ('node', 'business', 'current_version', 'sys_status')
