...
pddata = pd.read_csv('cust_machine_test.csv', encoding='gbk',keep_default_na=False)
pd_columns = pddata.columns.values.tolist()
print(pd_columns)
...
for row in pddata.itertuples():
    ...
    for os_item in ['redhat','centos','ubuntu','windows','clear linux','clearos','gentoo','arch linux']:
        i = len(os_item)
