...
#ping监控
def ping_monitor_task(verify={"loss": 25, "rtt": 800}):
    ...
    # else:
    #     sort_df = pddata
    ping_machineSet = tmodels.ShelfMachine.objects.filter(is_active='1', is_monitor='1').values('inner_ip', 'engine_room','cabinet','unit','server_model','serial_number','owner_id')
    error_list = []
    try:
        ...
        check_result_list = pool.map(ping_server_info, ping_machineSet)
