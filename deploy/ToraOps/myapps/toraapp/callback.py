...
#             'dukeqin@n-sight.com.cn','zhangwei@n-sight.com.cn']

oper_mails = getattr(settings, 'Tora_Oper_Group','[]')
mark_mails = getattr(settings, 'Tora_Market_Group','[]')

all_mails = oper_mails + mark_mails
...
def send_shelf_apply_mail(taskflow_obj):
    ...
    ownershipStr = dict(tmodels.OWNER_CHOICES)[taskflow_data['ownership']]
...
def send_shelf_mail(taskflow_data):
    ...
    node_mail_content = tdc.get_node_cfg_module(node_no)
...
#增加修改node增加时的操作
# @receiver(signals.post_init, sender=tmodels.NodeInfo)
# def modify_NodeInfo_post_init(sender, instance, **kwargs):
#     print("post_init_node_no:", instance.node_no)
#     instance.__original_no = instance.node_no



@receiver(signals.post_save, sender=tmodels.NodeInfo)
def modify_NodeInfo(sender, instance, created, **kwargs):
    print(type(kwargs))
    print("instance.node_no:", instance.node_no)
    #创建node成功，自动增加数据库监控配置项
    if created:
        print("增加新的NODE！")
        business_list = instance.business
        if business_list:
            for business in business_list:
                dbcfg_obj, is_created = mmodels.ToraDatabaseMonitorCfg.objects.get_or_create(
                                        node_no=instance.node_no,
                                        business=business)
                if is_created:
                    logger.info("自动增加数据库监控配置项成功！")
